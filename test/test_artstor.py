...
def test_artstor_data_provider_move_prop():
    """
    Move aggregatedCHO/spatial to dataProvider
    """
   ...
   "id": "6ae54cee603f75c275fd913e04c49a3f"
    ...
    url = server() + "copy_prop?prop=aggregatedCHO%2Fspatial&to_prop=dataProvider&create=True&remove=True"
    resp, content = H.request(url, "POST", body=INPUT_JSON)
    assert str(resp.status).startswith("2")
    data = json.loads(content)
    assert "dataProvider" in data, "No dataProvider field in document"
    assert data["dataProvider"] == EXPECTED_DATA_PROVIDER
    assert "aggregatedCHO" in data and "spatial" not in data["aggregatedCHO"], "spatial is left in document, but must be removed"
...
def test_artstor_data_provider_copy_prop_and_cleanup():
    """
    Copy aggregatedCHO/spatial as cleaned dataProvider
    """
